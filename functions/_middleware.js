export async function onRequest(context){
  ...
  if(isHome){
    ...
    const bridgeTag='<script src="/supabase-user-bridge.js?v=20260904-4"></script>';
    ...
    body=body.includes('</body>')?body.replace('</body>','<script src="/realtime-queue-refresh.js?v=20260904-4"></script></body>'):body+'<script src="/realtime-queue-refresh.js?v=20260904-4"></script>';
    ...
    body=body.replace(/src=["'](?:\.\/)?youtube-request-mapping\.js(?:\?[^"']*)?["']/g,'src="/youtube-request-mapping.js?v=20260904-3"');
  ...
  if(url.pathname==='/'||url.pathname.endsWith('.html')){
    ...
    const injection=hasYtMapping?'<script src="/youtube-request-mapping-batch-v2.js?v=20260904-3"></script>':'<script src="/youtube-request-mapping.js?v=20260904-3"></script><script src="/youtube-request-mapping-batch-v2.js?v=20260904-3"></script>';
  ...
  if(isPlayer){
    const sb='<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>',ps='<script src="/player-supabase.js?v=20260904-4"></script>';
    body=body.replace(/<script[^>]+src=["']https:\/\/cdn\.jsdelivr\.net\/npm\/@supabase\/supabase-js[^"']*["'][^>]*><\/script>/gi,'');
    body=body.includes('</head>')?body.replace('</head>',sb+'</head>'):body+sb;
