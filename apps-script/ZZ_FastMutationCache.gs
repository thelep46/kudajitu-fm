var KUDA_ID_INDEX_CACHE_='kudajitu_id_index_cache_v2';
...
function kudaFastAppendIndexDate_(items,startRow){if(typeof kudaFastIndexAfterAppend_!=='function'||!items||!items.length)return;try{kudaFastIndexAfterAppend_(items,startRow,items.length)}catch(e){}}
(function(){
  if(typeof appendIfMissing_==='function'&&!appendIfMissing_.__kudaFastMutationWrapped){
    var wrappedAppend=function(sheet,items){
      ...
      if(rows.length){sheet.getRange(start,1,rows.length,9).setValues(rows);if(typeof autoMapRequestsYouTube_==='function')autoMapRequestsYouTube_(rows.map(function(r){return{title:r[2],artist:r[3],note:r[4]};}));kudaFastAppendIndexDate_(rows.map(function(r){return{timestamp:r[5]};}),start)}
      kudaIdIndexPut_(idx);
