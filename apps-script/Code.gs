...
function clearCaches_(keepIdCache){
  try{
...
function autoMapRequestsYouTube_(items){
  ...
  (items||[]).forEach(function(item){
    var video=extractYouTubeId_(item&&item.note),title=String(item&&item.title||'').trim(),artist=String(item&&item.artist||'').trim();
    if(!video||!title)return;
    var key=kudaYoutubeKey_(title,artist);pending.push({key:key,title:title,artist:artist,video:video});seen[key]=pending[pending.length-1];
  });
  var keys=Object.keys(seen);if(!keys.length)return 0;
  var s=kudaYoutubeMapSheet_(),last=s.getLastRow(),rows=last>=2?s.getRange(2,1,last-1,4).getValues():[],index={};
  for(var i=0;i<rows.length;i++){var k=String(rows[i][0]||'');if(k)index[k]=i;}
  var changed=false,updates=0,appends=[];
  keys.forEach(function(key){var item=seen[key];if(Object.prototype.hasOwnProperty.call(index,key)){
    var rowIndex=index[key],row=rows[rowIndex],next=[key,item.title,item.artist,item.video];
    if(String(row[0]||'')!==next[0]||String(row[1]||'')!==next[1]||String(row[2]||'')!==next[2]||String(row[3]||'')!==next[3]){rows[rowIndex]=next;changed=true;updates++;}
  }else{appends.push([key,item.title,item.artist,item.video]);}});
  if(changed&&rows.length)s.getRange(2,1,rows.length,4).setValues(rows);
  if(appends.length)s.getRange(Math.max(2,1+rows.length+1),1,appends.length,4).setValues(appends);
  if((changed||appends.length)&&typeof kudaYoutubeCacheClear_==='function')kudaYoutubeCacheClear_();
  return updates+appends.length;
}
function appendIfMissing_(sheet,items){
  ...
  try{
    ...
    var accepted=[],rows=[],last=sheet.getLastRow(),appendStart=last+1,index={};
    if(last>=2){
      var ids=sheet.getRange(2,1,last-1,1).getValues();
      for(var i=0;i<ids.length;i++){var existingId=String(ids[i][0]||'');if(existingId)index[existingId]=i+2;}
    }
    items.forEach(function(item){
      var id=String(item.id||generateId_());
      if(index[id]){accepted.push(id);return}
      index[id]=appendStart+rows.length;
      accepted.push(id);
    ...
    if(rows.length)autoMapRequestsYouTube_(rows.map(function(r){return{title:r[2],artist:r[3],note:r[4]};}));
