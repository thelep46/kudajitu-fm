...
function appendIfMissing_(sheet,items){var accepted=[],rows=[],index=buildIdRowIndex_(sheet),appendStart=sheet.getLastRow()+1;items.forEach(function(item){var id=String(item.id||generateId_());if(index[id]){accepted.push(id);return}index[id]=appendStart+rows.length;accepted.push(id);rows.push([id,String(item.requester||''),String(item.title||''),String(item.artist||''),String(item.note||''),item.timestamp||new Date().toISOString(),normalizeStatus_(item.status||'pending'),Number(item.votes||1),'']);});if(rows.length)sheet.getRange(appendStart,1,rows.length,9).setValues(rows);if(rows.length)autoMapRequestsYouTube_(rows.map(function(r){return{title:r[2],artist:r[3],note:r[4]};}));return{accepted:accepted,added:rows.length,existing:accepted.length-rows.length};}
