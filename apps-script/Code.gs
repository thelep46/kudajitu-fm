...
function mutate_(sheet,p){
  ...
  if(action==='updateStatus'||action==='markPlayed'){
    var id=String(p.id||'').trim(),row=findRowById_(sheet,id);
    if(row<0)return{success:false,action:'updateStatus',message:'ID request tidak ditemukan: '+id};
    var st=normalizeStatus_(p.status||'played'),playedAt=st==='played'?new Date().toISOString():'';
    sheet.getRange(row,7,1,3).setValues([[st,sheet.getRange(row,8).getValue(),playedAt]]);
    clearCaches_();
    return{success:true,action:'updateStatus',id:id,row:row,status:st,playedAt:playedAt,message:st==='played'?'Request ditandai sudah diputar.':'Request dikembalikan ke antrean.'};
  }
  if(action==='updateStatuses'){
    var ids=String(p.ids||'').split(',').map(function(x){return x.trim()}).filter(Boolean),last=sheet.getLastRow();
    if(last<2)return{success:true,action:'updateStatuses',updated:0};
    var idRows=sheet.getRange(2,1,last-1,1).getValues(),values=sheet.getRange(2,7,last-1,3).getValues(),wanted={};
    ids.forEach(function(x){wanted[String(x)]=true});
    var st=normalizeStatus_(p.status||'played'),playedAt=st==='played'?new Date().toISOString():'',changed=0;
    for(var i=0;i<idRows.length;i++){var rid=String(idRows[i][0]||'');if(rid&&wanted[rid]){values[i][0]=st;values[i][2]=playedAt;changed++}}
    if(changed)sheet.getRange(2,7,last-1,3).setValues(values);
    clearCaches_();
    return{success:true,action:'updateStatuses',updated:changed};
  }
  if(action==='delete'){
...
function doGet(e){
  ...
  try{
    ...
    if(cached!==null)data=cached;
      ...
      data=filterStatus_(rows,status);cachePut_(key,data);
    }
    return respond_(cb,{success:true,action:'data',cached:cached!==null,cacheSeconds:CACHE_SECONDS,range:range,status:status,timezone:TIMEZONE,today:todayKey_(),yesterday:yesterdayKey_(),count:data.length,data:data});
