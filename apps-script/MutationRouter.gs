...
function kudaSaveYoutubeMapping_(p){kudaRequireAdmin_(p);var title=String(p.title||'').trim(),artist=String(p.artist||'').trim(),video=String(p.videoId||'').trim();if(!title)return{success:false,message:'Judul lagu wajib diisi.'};if(!/^[A-Za-z0-9_-]{11}$/.test(video))return{success:false,message:'Video ID YouTube tidak valid.'};var key=kudaYoutubeKey_(title,artist),s=kudaYoutubeMapSheet_(),last=s.getLastRow();if(last>=2){var rows=s.getRange(2,1,last-1,4).getValues();for(var i=0;i<rows.length;i++)if(String(rows[i][0]||'')===key){s.getRange(i+2,2,1,3).setValues([[title,artist,video]]);kudaYoutubeCacheClear_();return{success:true,message:'Mapping YouTube diperbarui.',key:key,videoId:video}}}s.appendRow([key,title,artist,video]);kudaYoutubeCacheClear_();return{success:true,message:'Mapping YouTube disimpan.',key:key,videoId:video}}
